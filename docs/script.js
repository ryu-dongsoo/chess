class ChessGame {
    ...
    initializeStockfish() {
        console.log('강화된 AI 엔진 초기화 - 사람이 절대 이길 수 없는 수준으로 설정됨');
...
if (colDiff === 1 && rowDiff === direction && targetPiece) {
    ...
    setStockfishDifficulty() {
        ...
        // Stockfish 설정 - 정말 이길 수 없을 정도로 어렵게 설정
        ...
        this.stockfish.postMessage('setoption name Threads value 4'); // 더 많은 스레드 사용
        this.stockfish.postMessage('setoption name Hash value 1024'); // 더 큰 해시 테이블
        ...
        console.log('Stockfish 난이도 설정 완료 - 정말 이길 수 없을 정도로 어려움!');
    }
    ...
    createGameIssue(roomId, playerName) {
        const gameData = {
            roomId: roomId,
            players: [{name: playerName, color: 'white', joinedAt: Date.now()}],
        ...
        console.log('방 생성자 팀 할당:', playerName, '-> 흰색');
        this.playerColor = 'white';
    }
    ...
    sendMoveToGitHub() {
        ...
        if (gameData) {
            const game = JSON.parse(gameData);
            // 전체 상태를 저장
            game.gameState = JSON.parse(JSON.stringify(this.board));
            game.currentPlayer = this.currentPlayer;
            game.moveHistory = JSON.parse(JSON.stringify(this.moveHistory));
            localStorage.setItem(gameKey, JSON.stringify(game));
            // 로그 등은 필요시 유지
        }
    }
